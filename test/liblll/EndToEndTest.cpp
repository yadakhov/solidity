	...
	char const* sourceCode = R"(
		(returnlll
			(seq
				...
				(set 'k (add (add (ref 'x) (ref 'y)) (ref 'z)))
				;; variables can be referred to by (get 'x) or simply x too
				(return (add (add x (add y z)) (get 'k)))))
	)";
	compileAndRun(sourceCode);
	BOOST_CHECK(callFallback() == encodeArgs(u256(493)));
}
