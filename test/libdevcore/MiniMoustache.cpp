...
{
	string templ = "<b>";
	MiniMoustache m(templ);
	BOOST_CHECK_THROW(m.render(), MiniMoustacheError);
}
...
BOOST_AUTO_TEST_CASE(empty_list)
{
	...
	string result = MiniMoustache(templ)("b", vector<MiniMoustache::StringMap>{}).render();
...
BOOST_AUTO_TEST_CASE(list)
{
	...
	list[1]["g"] = "2GE";
	list[1]["h"] = "2H";
	string result = MiniMoustache(templ)("b", list).render();
...
BOOST_AUTO_TEST_CASE(recursive_list)
{
	...
	list[0]["g"] = "<x>";
	string result = MiniMoustache(templ)("x", "X")("b", list).render();
