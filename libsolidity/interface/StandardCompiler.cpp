...
Json::Value compileLLL(Json::Value const& _input)
{
	...
	output["contracts"][""] = Json::objectValue;
	output["contracts"][""]["parsed"] = dev::eth::parseLLL(_input["source"].asString());
	output["contracts"][""]["assembly"] = dev::eth::compileLLLToAsm(_input["source"].asString(), optimize, &errorList);
	output["contracts"][""]["bytecode"] = toHex(dev::eth::compileLLL(_input["source"].asString(), optimize, &errorList));
