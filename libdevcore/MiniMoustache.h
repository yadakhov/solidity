...
DEV_SIMPLE_EXCEPTION(MiniMoustacheError);
...
public:
	...
	explicit MiniMoustache(std::string const& _template);

	/// Sets a single parameter, <paramName>.
	MiniMoustache& operator()(std::string const& _parameter, std::string const& _value);
	/// Sets a list parameter, <#listName> </listName>.
	MiniMoustache& operator()(
